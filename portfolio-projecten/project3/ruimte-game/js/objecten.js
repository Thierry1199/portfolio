...
class Player extends GameObject {
	constructor(posX, posY) {
		// Constante snelheden voor de raket
		const HORIZONTAL_SPEED = 3;  // Basis horizontale snelheid
		const VERTICAL_SPEED = 5;    // Basis verticale snelheid
		super(posX, posY, HORIZONTAL_SPEED, 0, 116, 112, 'img/player.png');
		this.VERTICAL_SPEED = VERTICAL_SPEED;  // Sla op voor gebruik in moveUp/moveDown
		this.lives = 3;
		...
		this.currentSpeed = HORIZONTAL_SPEED;  // Houd de huidige snelheid bij
	}
	...
	loseLife() {
		...
		// Verbeterde visuele feedback voor leven verloren
		this.lifeMessage = {
			text: "-1 LEVEN",  // Duidelijke Nederlandse tekst in plaats van emoji
	...
	draw() {
		if (this.visible) {
			...
			// Toon de huidige snelheid
			ctx.save();
			ctx.fillStyle = 'white';
			ctx.font = '14px Arial';
			ctx.textAlign = 'center';
			ctx.fillText(`Snelheid: ${this.currentSpeed}`, this.X + this.width / 2, this.Y - 10);
			ctx.restore();
		}

		// Teken life message met verbeterde stijl
		if (this.lifeMessage) {
			ctx.save();
			ctx.globalAlpha = this.lifeMessage.opacity;
			ctx.fillStyle = '#ff3333';  // Helderrood
			ctx.strokeStyle = 'white';  // Witte outline voor betere leesbaarheid
			ctx.lineWidth = 3;
			ctx.font = 'bold 36px Arial';
			ctx.textAlign = 'center';
			ctx.strokeText(this.lifeMessage.text, this.lifeMessage.x, this.lifeMessage.y);
